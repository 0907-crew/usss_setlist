...
<body>
    ...
    function getHeaderClass(artist) {{
        ...
        if (cleanArtist === 'うらたぬき') return 'header-urata';
        if (cleanArtist === '志麻') return 'header-shima';
        if (cleanArtist === 'あほの坂田。' || cleanArtist === 'となりの坂田。') return 'header-sakata';
        if (cleanArtist === 'センラ') return 'header-senra';
        return 'header-usss';
    }}
...
<!-- フッター部分 -->
    <footer class="text-center py-4 text-muted small border-top mt-5">
        ...
        <a href="https://marshmallow-qa.com/1bem41p9vsq1xfn?t=9wMMKH&utm_medium=url_text&utm_source=promotion" target="_blank" rel="noopener noreferrer" class="btn btn-outline-secondary btn-sm">
            マシュマロでメッセージを送る
        </a>
    </footer>
</body>
